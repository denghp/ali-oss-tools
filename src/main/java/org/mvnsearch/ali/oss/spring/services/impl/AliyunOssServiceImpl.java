...
public class AliyunOssServiceImpl implements AliyunOssService {
    ...
    public String get(OSSUri objectUri, String destFilePath) throws Exception {
        ...
        if (ossObject != null) {
            if (destFile.isDirectory() && !objectUri.isDirectory()) {
                destFile = new File(destFile, ossObject.getKey());
            } else {
                FileUtils.forceMkdir(new File(destFilePath + File.separator+ objectUri.getPath()));
                return objectUri.getPath();
            }
    ...
    public boolean getAll(String bucketName, String srcPath, String destPath) throws Exception {
        if(StringUtils.isBlank(srcPath)) {
            log.warn("get src path is not found.");
            return false;
        }
        if (StringUtils.isBlank(destPath)) {
            log.warn("save destPath is not found.");
            return false;
        }
        ...
        if (srcPath != null && !srcPath.equals("*")) {
            ObjectListing objectListing = list(bucketName, srcPath, MAX_OBJECTS);
            for (OSSObjectSummary objectSummary : objectListing.getObjectSummaries()) {
                objectSummary.setBucketName(bucketName);
                get(new OSSUri(bucketName,objectSummary.getKey()),destPath);
            }
        } else if (srcPath.equals("*")) {
            ObjectListing objectListing = list(bucketName, "", MAX_OBJECTS);
            for (OSSObjectSummary objectSummary : objectListing.getObjectSummaries()) {
                objectSummary.setBucketName(bucketName);
                get(new OSSUri(bucketName,objectSummary.getKey()),destPath);
            }

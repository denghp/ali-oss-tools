...
public class OssOperationCommands implements CommandMarker {
    ...
    public String get(@CliOption(key = {"dest"}, mandatory = false, help = "Local file or directory path") File localFilePath,
     ...
     * @return message
     */
    @CliCommand(value = "getAll", help = "Retrieve OSS list object and save it to local file system")
    public String getAll(@CliOption(key = {"dest"}, mandatory = false, help = "Local file or directory path") String localFilePath,
                      @CliOption(key = {"src"}, mandatory = true, help = "buck path") String srcPath) {
        if (currentBucket == null) {
            return wrappedAsYellow("Please select a bucket!");
        }
        try {
            aliyunOssService.getAll(currentBucket.getBucket(), srcPath, localFilePath);
            return MessageFormat.format("AllObjects {0} saved to {1} )", srcPath, localFilePath);
        } catch (Exception e) {
            log.error("get", e);
            return e.getMessage();
        }
    ...
    public String put(@CliOption(key = {"source"}, mandatory = true, help = "Local file or directory path") File sourceFile,
                      ...
                      @CliOption(key = {"headers"}, mandatory = false, help = "Metadata keys Cache-Control:max-age=1800&Content-Encoding:utf-8") final String headers,
        ...
        try {
            if (sourceFile.isDirectory()) {
                int count = uploadDirectory(currentBucket.getBucket(), StringUtils.defaultIfEmpty(objectKey, ""), sourceFile, false, zip, headers);
                ...
                if (StringUtils.isNotBlank(headers)) {
                    setMetaData(destObjectUri, headers);
                }
    ...
    /**
     * 设置header,e.g: key1:value1&key2:value2
     *
     * @param headers
     * @return
     */
    private boolean setMetaData(OSSUri destObjectUri, String headers) {
        log.info("set {} meta info {}, starting...", destObjectUri, headers);
        try {
            if (StringUtils.isNotBlank(headers)) {
                String[] items = headers.split("&");
                if (items != null && items.length > 0) {
                    for (String item : items) {
                        String[] headerAttr = item.split(":");
                        if (headerAttr != null && headerAttr.length > 1) {
                            String key = headerAttr[0];
                            String value = headerAttr[1];
                            log.info("set {} meta info {}:{}", destObjectUri, key, value);
                            aliyunOssService.setObjectMetadata(destObjectUri, key, value);
                        }
                    }
                    log.info("set {} meta successfully...",destObjectUri);
                    return true;
                } else {
                    log.warn("This headers is INvalid. {}, valid set example : key1:value1&key2:value2" + headers);
                }
            }
        } catch (Exception e) {
            log.error("put", e);
     ...
     */
    private int uploadDirectory(String bucket, String destFilePath, File sourceDir, boolean synced, Boolean zip, String headers) throws Exception {
    ...
    public String sync(@CliOption(key = {"source"}, mandatory = false, help = "local directory") @Nullable File sourceFile,
        ...
        try {
            if (sourceFile.isDirectory()) {
                int count = uploadDirectory(bucketName, StringUtils.defaultIfEmpty(objectPath, ""), sourceFile, true, zip, null);
                return count + " files uploaded!";
            } else {
